...
SCENE_RE = re.compile(r"^theta(\d+)_phi(\d+)_scene_distances_viz\.png$")

def parse_args():
    p = argparse.ArgumentParser(description="Create interactive distance grid from CSV + scene images.")
...
def read_csv_view_summaries(csv_path):
    """
    Reads the simplified CSV (viewpoint, distance). Returns list of (viewpoint_str, occl_float).
...
def main():
    p = argparse.ArgumentParser(
        description="Create interactive distance grid from CSV + scene images."
    )
    ...
    csv_path = os.path.join(out_dir, "distance_summary.csv")
    scene_dir = os.path.join(base_dir, "distance")

    if not os.path.isfile(csv_path):
        raise FileNotFoundError(f"CSV not found: {csv_path}")

    os.makedirs(out_dir, exist_ok=True)
    grid_path = os.path.join(out_dir, "distance_grid.png")
    ...
    cbar = plt.colorbar(im, ax=ax_heat, label="proximity (px)", fraction=0.045, pad=0.02)
    cbar.ax.tick_params(labelsize=CBAR_FS)

    ax_heat.set_title("Per-view average proximity", fontsize=SMALL_TITLE_FS)
    ...
    def on_hover(event):
        ...
        try:
            ...
            phi_val = phis_arr[j]
            occ_val = grid[i, j]
            occ_text = "N/A" if np.isnan(occ_val) else f"{occ_val:.2f} px"

            ax_img.set_title(
                f"theta={theta_val}°, phi={phi_val}° | proximity: {occ_text}",
